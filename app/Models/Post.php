...
//            ->orWhere('userName', 'like', "%" . $searchWord . "%")
    ...
    public function getShowOrdersAttribute()
    {
        if ($this->number_of_requests == 0) {
            return '<button  type="button" class="btn bg-gradient-faded-info w-40 mb-0 toast-btn disabled " data-toggle="tooltip" data-placement="top" rel="tooltip" title=" ' . $this->number_of_requests . '" onclick="ShowOrders(\'' . route('posts.ShowOrders', $this->id) . '\',this)"><i class="bi bi-list-ul"></i>post orders</button>';

        } else
            return '<button type="button" class="btn bg-gradient-faded-info w-40 mb-0 toast-btn" data-toggle="tooltip" data-placement="top" rel="tooltip" title=" ' . $this->number_of_requests . '" onclick="ShowOrders(\'' . route('posts.ShowOrders', $this->id) . '\',this)"><i class="bi bi-list-ul"></i>post orders</button>';
    }
    public function getDeletePostAttribute()
    {
        if ($this->second_user == null) {
            return '<button  type="button" class="btn bg-gradient-faded-danger w-40 mb-0 toast-btn " data-toggle="tooltip"
             data-placement="top" rel="tooltip" title="Delete '.$this->name.'" onclick="deleteItem(\''.route('posts.destroy',$this->id).'\')"><i class="fa fa-trash"></i></button>';
    ...
    public function getShowIsDonationPostAttribute()
    {
        ...
        return ' <button class="btn bg-gradient-faded-info w-100 mb-0 toast-btn" type="button" data-target="successToast">Beneficiary</button>';
    }
...
//-------------------------------start scope -------------------------------------
    ...
    {
//        dd($searchWord,!empty($searchWord) , $searchWord == 0,);
        if (!empty($searchWord) || !is_null($searchWord)) {
...
//Api
    ...
    public function getCategoryNameAttribute($request)
    {
//        dd($request);
        return $this->category ?($request?($request=='ar'?$this->category->name_ar:$this->category->name): ucfirst($this->category->name)) : 'category not found';
